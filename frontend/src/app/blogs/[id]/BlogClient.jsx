...
const onSubmitHandler = async (e) => {
  ...
  // Track view after blog data is loaded
  useEffect(() => {
    if (!data) return;
    ...
    // Fast-path: already tracked this blog in this session
    const trackedKey = `tracked_${data._id}`;
    if (sessionStorage.getItem(trackedKey)) return;

    // Get subscriber email if stored
    const subscriberEmail = localStorage.getItem('subscriberEmail') || '';

    // Fire-and-forget
    axios.post(`${baseURL}/api/blog/track-view`, {
      blogId: data._id,
      ...
      // Silently ignore — analytics must not affect reading experience
    });
  }, [data]);
